...
def main() -> None:
    ...
    if args.question_count > len(questions):
        raise ValueError(
            f"question-count {args.question_count} exceeds dataset size {len(questions)}"
        )
    ...
    with httpx.Client(
        ...
        for question in questions[: args.question_count]:
